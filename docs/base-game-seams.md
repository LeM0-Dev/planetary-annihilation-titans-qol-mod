...
| `gw_play` model vs game object | `currentStar` | `model.currentStar()` EXISTS but is not reliably set in solo play — always read `model.game().currentStar()` (found the hard way: the Game Info intel persist silently threw every tick) |
| `gw_play` model | `uberId` (session observable) | The HOST also has a coopPlayerInventoryData record — dedupe against uberId; editing the live inventory ≠ editing the host's record snapshot |
| GW card modules | AMD singletons via `requireGW(['cards/<id>'])` | The dealer holds the same cached object — wrapping `module.deal()` (ban list) affects every future roll; `summarize()/describe()/icon()` power the card browser. `gwc_minion` needs a per-instance `minion` payload — never grant a bare id |
| `gw_play` DOM | `.bottom-bar` social overlay | Swallows clicks over the inventory tray (incl. the TECHS label) — interactive elements there need a fixed-position hotspot layered above it |
| `gw_play` co-op ops | `sendCampaignAction` / `registerCampaignViewerOperatorHandler` | Custom host→viewer operators (pin sync). Scene mods load BEFORE the gw_play model exists — never put `model.*` in `requires` for this scene; wire lazily |
